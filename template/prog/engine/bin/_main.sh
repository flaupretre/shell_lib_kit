...
{
typeset has_run string func fline fstart nb funcs selfuncs
...
for string ; do
	if [ "X$string" = 'X*' -o "X$string" = 'Xall' ] ; then
		funcs=`typeset -F | sed 's/^declare -f //' | grep '^@SLK_PREFIX@_' | sed 's/^@SLK_PREFIX@_//' | sort`
	else
		funcs=`typeset -F | sed 's/^declare -f //' | grep '^@SLK_PREFIX@_' | sed 's/^@SLK_PREFIX@_//' | grep "$string" | sort`
	fi
...
export _@SLK_LIBNAME_UC@_PPC_DIR=$_@SLK_LIBNAME_UC@_BASE/ppc
export _@SLK_LIBNAME_UC@_SCRIPT_SH=$_@SLK_LIBNAME_UC@_PPC_DIR/script.sh
